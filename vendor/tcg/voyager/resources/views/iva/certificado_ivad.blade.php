...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
	...
	.border-lb {border: 1px solid #323232; border-width: 0 1 0px 0px;}
    </style>
</head>
<body>
<table width="550" cellpadding="5" cellspacing="0" align="center" class="Estilo5">
  <!-- <tr class="texto">
          <td width="100%" class="Estilo1" colspan="3" style="border: 0">
            <div class="header">
              <img max-width="50%" max-height="20"  src="{{url('images/empresa/'.$empresa->logotipo_color)}}">
              <span class="logotext">CERTIFICADO DE RETENCION DE I V A</span>
            </div>
         </td>
  </tr> -->
  <tr class="texto">
  ...
  <tr>
    <td height="" colspan="3" valign="top" style="border: 0">
      ...
      @if ($iva->id_empresa == $empresa->id_empresa)
      @if($loop->last)
         <tr>
            <td class="border-lb" style="padding-bottom: 70px;">
            	<div align="center">
            		@if ($iva->Periodo == 1)
				    Enero - Febrero
            ...
            <td class="border-lb" style="padding-bottom: 70px;">{{$iva->Concepto}}</td>
            <td  class="border-lb" style="padding-bottom: 70px;"><div align="right">${{number_format($iva->Base,0,',','.')}}</div></td>
            <td  class="border-lb" style="padding-bottom: 70px;"><div align="right">${{number_format($iva->Iva,0,',','.')}}</div></td>
            <td  class="border-lb" style="padding-bottom: 70px;"><div align="right">{{$iva->Porcentaje}}</div></td>
            <td  class="border-lb" style="padding-bottom: 70px;"><div align="right">${{number_format($iva->Retenido,0,',','.')}}</div></td>
        </tr>
      @else
        <tr>
            ...
            <td class="border-lb">{{$iva->Concepto}}</td>
            <td class="border-lb" ><div align="right">${{number_format($iva->Base,0,',','.')}}</div></td>
            <td class="border-lb" ><div align="right">${{number_format($iva->Iva,0,',','.')}}</div></td>
            <td class="border-lb" ><div align="right">{{$iva->Porcentaje}}</div></td>
            <td class="border-lb" ><div align="right">${{number_format($iva->Retenido,0,',','.')}}</div></td>
        </tr>
        @endif
        @endif
        <?php
